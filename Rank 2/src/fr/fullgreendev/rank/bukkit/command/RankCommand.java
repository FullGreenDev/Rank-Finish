package fr.fullgreendev.rank.bukkit.command;
...
import fr.fullgreendev.rank.RankList;
	...
	public boolean onCommand(CommandSender s, Command c, String l, String[] a) {
 		if(s instanceof Player)
 			if(rank.hasPowerInf((Player)s, 90))
 				return sendMessage(s, "§cVous n'avez pas la permission.");
		...
		OfflinePlayer target = Bukkit.getOfflinePlayer(a[0]);		
 		...
 		if(target.isOnline()){
 			sendMessage(target.getPlayer(), "§9Votre grade a été modifié.");
